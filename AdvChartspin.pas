...
const
  ...
  // 1.4.4.5 : Fixed : issue with AllowNullValue in TDBAdvChartSpinEdit
  // 1.4.4.6 : Fixed : issue with '-' handling in descending class TDBAdvChartSpinEdit
  ...
  // 1.4.6.0 : Improved : handling of AutoEdit = false in TDBAdvChartSpinEdit
  ...
  // 1.6.1.1 : Fixed : Issue with alignment with TAdvChartSpinEdit with label active
...
type
  ...
  TAdvChartTimerSpeedButton = class;
...
{ TAdvChartSpinButton }
  ...
  TAdvChartSpinButton = class (TWinControl)
  private
    FUpButton: TAdvChartTimerSpeedButton;
    FDownButton: TAdvChartTimerSpeedButton;
    FFocusedButton: TAdvChartTimerSpeedButton;
    ...
    function CreateButton: TAdvChartTimerSpeedButton;
    ...
    procedure SetFocusBtn (Btn: TAdvChartTimerSpeedButton);
...
{ TAdvChartSpinEdit }
  ...
  TAdvChartSpinType = (sptNormal,sptFloat,sptDate,sptTime,sptHex);

  {$IFDEF DELPHIXE2_LVL}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvChartSpinEdit = class(TCustomMaskEdit)
  private
    FFlat: Boolean;
    ...
    FButton: TAdvChartSpinButton;
    FEditorEnabled: Boolean;
    FDirection: TSpinDirection;
    FSpinType: TAdvChartSpinType;
    FPrecision: integer;
    ...
    procedure SetSpinType(const Value: TAdvChartSpinType);
  ...
  public
    ...
    property Button: TAdvChartSpinButton read FButton;
    procedure IncSmart;
    procedure DecSmart;
    procedure SetBounds(ALeft, ATop, AWidth, AHeight: Integer); override;
    procedure Init;
  ...
  published
    ...
    property SpinType: TAdvChartSpinType read FSpinType write SetSpinType default sptNormal;
  ...
  TAdvChartMaskSpinEdit = class(TAdvChartSpinEdit)
  published
    property EditMask;
  end;

{ TAdvChartTimerSpeedButton }

  TTimeBtnState = set of (tbFocusRect, tbAllowTimer);

  TButtonDirection = (bdLeft,bdRight,bdUp,bdDown);

  TAdvChartTimerSpeedButton = class(TSpeedButton)
  private
    FRepeatTimer: TTimer;
...
constructor TAdvChartSpinButton.Create(AOwner: TComponent);
begin
...
function TAdvChartSpinButton.CreateButton: TAdvChartTimerSpeedButton;
begin
  Result := TAdvChartTimerSpeedButton.Create (Self);
...
procedure TAdvChartSpinButton.Notification(AComponent: TComponent;
...
procedure TAdvChartSpinButton.AdjustSize(var W, H: Integer);
var
  bh: integer;
begin
...
procedure TAdvChartSpinButton.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  W, H: Integer;
begin
  W := AWidth;
...
procedure TAdvChartSpinButton.WMSize(var Message: TWMSize);
var
  W, H: Integer;
begin
  inherited;
...
procedure TAdvChartSpinButton.WMSetFocus(var Message: TWMSetFocus);
begin
  FFocusedButton.TimeBtnState := FFocusedButton.TimeBtnState + [tbFocusRect];
  FFocusedButton.Invalidate;
end;

procedure TAdvChartSpinButton.WMKillFocus(var Message: TWMKillFocus);
begin
  FFocusedButton.TimeBtnState := FFocusedButton.TimeBtnState - [tbFocusRect];
  FFocusedButton.Invalidate;
end;

procedure TAdvChartSpinButton.KeyDown(var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_UP:
      begin
...
procedure TAdvChartSpinButton.BtnMouseDown (Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Button = mbLeft then
  begin
    SetFocusBtn (TAdvChartTimerSpeedButton (Sender));
...
procedure TAdvChartSpinButton.BtnClick(Sender: TObject);
begin
...
procedure TAdvChartSpinButton.SetDirection(const Value: TSpinDirection);
begin
...
procedure TAdvChartSpinButton.SetFocusBtn (Btn: TAdvChartTimerSpeedButton);
begin
...
procedure TAdvChartSpinButton.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  Message.Result := DLGC_WANTARROWS;
end;

procedure TAdvChartSpinButton.Loaded;
var
  W, H: Integer;
begin
  inherited Loaded;
...
function TAdvChartSpinButton.GetUpGlyph: TBitmap;
begin
  Result := FUpButton.Glyph;
end;

procedure TAdvChartSpinButton.SetUpGlyph(Value: TBitmap);
begin
  if Value <> nil then
    FUpButton.Glyph := Value
  else
  begin
    FUpButton.Glyph.Handle := LoadBitmap(HInstance, 'AdvChartSpinUp');
    FUpButton.NumGlyphs := 1;
    FUpButton.Invalidate;
  end;
end;

function TAdvChartSpinButton.GetUpNumGlyphs: TNumGlyphs;
begin
  Result := FUpButton.NumGlyphs;
end;

procedure TAdvChartSpinButton.SetUpNumGlyphs(Value: TNumGlyphs);
begin
  FUpButton.NumGlyphs := Value;
end;

function TAdvChartSpinButton.GetDownGlyph: TBitmap;
begin
  Result := FDownButton.Glyph;
end;

procedure TAdvChartSpinButton.SetDownGlyph(Value: TBitmap);
begin
  if Value <> nil then
    FDownButton.Glyph := Value
  else
  begin
    FDownButton.Glyph.Handle := LoadBitmap(HInstance, 'AdvChartSpinDown');
    FUpButton.NumGlyphs := 1;
    FDownButton.Invalidate;
  end;
end;

function TAdvChartSpinButton.GetDownNumGlyphs: TNumGlyphs;
begin
  Result := FDownButton.NumGlyphs;
end;

procedure TAdvChartSpinButton.SetDownNumGlyphs(Value: TNumGlyphs);
begin
...
constructor TAdvChartSpinEdit.Create(AOwner: TComponent);
var
  i: integer;
begin
  inherited Create(AOwner);
  ...
  FButton := TAdvChartSpinButton.Create (Self);
  FButton.Width := 15;
  FButton.Height := 17;
...
destructor TAdvChartSpinEdit.Destroy;
begin
  FButton := nil;
  if FLabel <> nil then
  begin
...
procedure TAdvChartSpinEdit.Loaded;
var
  fcol: TColor;
begin
  inherited;
...
procedure TAdvChartSpinEdit.WMSetFocus(var Message: TWMSetFocus);
begin
  inherited;
...
procedure TAdvChartSpinEdit.WMKillFocus(var Message: TWMKillFocus);
begin
  inherited;
...
procedure TAdvChartSpinEdit.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
end;

procedure TAdvChartSpinEdit.KeyDown(var Key: Word; Shift: TShiftState);
var
  ss,sl: Integer;
  oldval: String;
begin
...
procedure TAdvChartSpinEdit.KeyPress(var Key: Char);
begin
...
function TAdvChartSpinEdit.Is3DBorderButton: Boolean;
begin
...
function TAdvChartSpinEdit.IsValidChar(var Key: Char): Boolean;
var
  dp: Integer;
begin
...
procedure TAdvChartSpinEdit.CreateParams(var Params: TCreateParams);
begin
...
procedure TAdvChartSpinEdit.CreateWnd;
begin
  inherited CreateWnd;
  SetEditRect;
end;

procedure TAdvChartSpinEdit.SetDirection(const value: TSpinDirection);
begin
  if Value <> FDirection then
  begin
    FDirection := Value;
...
procedure TAdvChartSpinEdit.SetDisabledBorder(const Value: Boolean);
begin
...
procedure TAdvChartSpinEdit.SetDisabledColor(const Value: TColor);
begin
...
procedure TAdvChartSpinEdit.SetEditorEnabled(NewValue:boolean);
begin
  FEditorEnabled := NewValue;
end;

procedure TAdvChartSpinEdit.SetEditRect;
var
  Loc: TRect;
  Dist : integer;
begin
...
procedure TAdvChartSpinEdit.SetBorder3D(const Value: Boolean);
begin
...
procedure TAdvChartSpinEdit.SetBorderColor(const Value: TColor);
begin
...
procedure TAdvChartSpinEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  if Assigned(FLabel) then
  begin
...
procedure TAdvChartSpinEdit.WMSize(var Message: TWMSize);
var
  MinHeight: Integer;
  Dist: Integer;
...
function TAdvChartSpinEdit.GetMinHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure TAdvChartSpinEdit.UpClick(Sender: TObject);
var
  ss,sl: Integer;
  oldval: string;
  dp: TDatePart;
...
function TAdvChartSpinEdit.DoCheckInt: boolean;
begin
  Result := (MinValue <> MaxValue) or (CheckMinValue <> CheckMaxValue) or (CheckMinValue and CheckMaxValue);
end;

function TAdvChartSpinEdit.DoMouseWheelDown(Shift: TShiftState;
...
function TAdvChartSpinEdit.DoMouseWheelUp(Shift: TShiftState;
  MousePos: TPoint): Boolean;
var
  Handled: boolean;
begin
...
function TAdvChartSpinEdit.DoCheckFloat: boolean;
begin
  Result := (MinFloatValue <> MaxFloatValue) or (CheckMinValue <> CheckMaxValue);
end;

function TAdvChartSpinEdit.DoCheckDate: boolean;
begin
  Result := (MinDateValue <> MaxDateValue) or (CheckMinValue <> CheckMaxValue);
end;

procedure TAdvChartSpinEdit.DownClick (Sender: TObject);
var
  ss,sl: Integer;
  oldval: string;
  dp: TDatePart;
...
procedure TAdvChartSpinEdit.WMPaste(var Message: TWMPaste);
var
  {$IFNDEF DELPHI_UNICODE}
  Data: THandle;
  content: PChar;
...
procedure TAdvChartSpinEdit.WMCut(var Message: TWMPaste);
begin
  if not FEditorEnabled or ReadOnly then
    Exit;
  inherited;
...
procedure TAdvChartSpinEdit.CMExit(var Message: TCMExit);
begin
...
function TAdvChartSpinEdit.GetValue: LongInt;
begin
  try
    if Trim(Text) = '' then
      Result := 0
...
function TAdvChartSpinEdit.CheckFloatValue (NewValue: Double): Double;
begin
  Result := NewValue;
...
function TAdvChartSpinEdit.GetFloatValue: Double;
begin
  try
	if Trim(Text) = '' then
      Result := 0
...
procedure TAdvChartSpinEdit.SetFloatValue (NewValue: Double);
begin
...
procedure TAdvChartSpinEdit.SetFocusBorder(const Value: Boolean);
begin
...
procedure TAdvChartSpinEdit.SetValue(NewValue: LongInt);
begin
  Text := IntToStr(CheckValue(NewValue));
  if not FEditorEnabled and AutoSelect then SelectAll;
end;

function TAdvChartSpinEdit.CheckValue(NewValue: LongInt): LongInt;
begin
  Result := NewValue;
  if not (csLoading in ComponentState) then
  begin
...
function TAdvChartSpinEdit.CheckDateValue (NewValue: TDateTime): TDateTime;
begin
  Result := NewValue;
  if not (csLoading in ComponentState) then
  begin
...
procedure TAdvChartSpinEdit.CMEnabledChanged(var Msg: TMessage);
begin
...
procedure TAdvChartSpinEdit.CMEnter(var Message: TCMGotFocus);
begin
...
procedure TAdvChartSpinEdit.SetSpinFlat(const value: boolean);
begin
...
procedure TAdvChartSpinEdit.SetSpinTransparent(const value: boolean);
begin
...
procedure TAdvChartSpinEdit.SetParent(AParent: TWinControl);
begin
  inherited;
  if FLabel <> nil then
...
procedure TAdvChartSpinEdit.SetPrecision(const Value: integer);
begin
  FPrecision := Value;
...
procedure TAdvChartSpinEdit.SetSpinType(const Value: TAdvChartSpinType);
var
...
function TAdvChartSpinEdit.GetTimeValue: TDateTime;
begin
  if Trim(Text) = '' then
    Result := 0
  else
...
procedure TAdvChartSpinEdit.SetTimeValue(const Value: TDateTime);
var
  ss: Integer;
begin
  FTimeValue := Value;
...
function TAdvChartSpinEdit.GetDateValue: TDateTime;
begin
...
procedure TAdvChartSpinEdit.SetDateValue(const Value: TDateTime);
var
  ss: Integer;
begin
  FDateValue := Value;
...
function TAdvChartSpinEdit.GetHexValue: integer;
var
 s:string;
 r: Integer;
begin
...
procedure TAdvChartSpinEdit.SetHexValue(const Value: integer);
var
  newtxt: string;
begin
...
function TAdvChartSpinEdit.DatePartAtCursor(Pos: integer): TDatePart;
type
...
procedure TAdvChartSpinEdit.DecSmart;
var
  ss,sp: Integer;
  incd: Integer;
  incf: Double;
...
procedure TAdvChartSpinEdit.IncSmart;
var
  ss,sp,dc: Integer;
  incd: Integer;
  incf: double;
...
procedure TAdvChartSpinEdit.UpdateLabel;
begin
...
procedure TAdvChartSpinEdit.UpdateLabelPos;
var
  tw: Integer;
  r: TRect;
begin
...
function TAdvChartSpinEdit.CreateLabel: TLabel;
begin
...
function TAdvChartSpinEdit.GetLabelCaption: string;
begin
  if FLabel <> nil then
...
procedure TAdvChartSpinEdit.SetLabelCaption(const Value: string);
begin
  if FLabel = nil then
    FLabel := CreateLabel;
...
procedure TAdvChartSpinEdit.SetLabelMargin(const Value: integer);
begin
  FLabelMargin := Value;
  if FLabel <> nil then
    UpdateLabel;
...
procedure TAdvChartSpinEdit.SetLabelPosition(const Value: TLabelPosition);
begin
...
procedure TAdvChartSpinEdit.SetLabelTransparent(const Value: boolean);
begin
...
function TAdvChartSpinEdit.GetVisible: boolean;
begin
  Result := inherited Visible;
end;

procedure TAdvChartSpinEdit.SetVisible(const Value: boolean);
begin
...
procedure TAdvChartSpinEdit.LabelFontChange(Sender: TObject);
begin
  if (FLabel <> nil) then
    UpdateLabel;
end;

procedure TAdvChartSpinEdit.SetLabelFont(const Value: TFont);
begin
  FLabelFont.Assign(Value);
end;


procedure TAdvChartSpinEdit.CNCtlColorEdit(var Message: TWMCtlColorEdit);
begin
  inherited;
...
procedure TAdvChartSpinEdit.CNCtlColorStatic(var Message: TWMCtlColorStatic);
begin
  inherited;
  if FTransparent then
    SetBkMode(Message.ChildDC, Windows.TRANSPARENT);
...
procedure TAdvChartSpinEdit.CNCommand(var Message: TWMCommand);
begin
...
procedure TAdvChartSpinEdit.CMFontChanged(var Message: TMessage);
begin
...
procedure TAdvChartSpinEdit.CMParentFontChanged(var Message: TMessage);
begin
  inherited;
...
procedure TAdvChartSpinEdit.WMPaint(var Msg:TWMPaint);
begin
  inherited;
  PaintEdit;
...
procedure TAdvChartSpinEdit.WMNCPaint(var Message: TMessage);
begin
  inherited;
...
procedure TAdvChartSpinEdit.DrawBorder;
var
  DC: HDC;
begin
...
procedure TAdvChartSpinEdit.DrawControlBorder(DC: HDC);
var
  ARect: TRect;
...
procedure TAdvChartSpinEdit.PaintEdit;
var
 DC: HDC;
 oldpen: HPen;
 loc: TRect;
...
procedure TAdvChartSpinEdit.WMEraseBkGnd(var Message: TWMEraseBkGnd);
var
  DC: HDC;
  i: integer;
  p: TPoint;
...
procedure TAdvChartSpinEdit.SetTransparent(const Value: Boolean);
begin
...
procedure TAdvChartSpinEdit.SetFlatRect(const Value: Boolean);
var
  loc: TRect;
begin
  if value then
...
procedure TAdvChartSpinEdit.SetFlat(const value: boolean);
begin
  if (csLoading in ComponentState) then
  begin
    FFlat := Value;
...
procedure TAdvChartSpinEdit.SetFlatLineColor(const Value: TColor);
begin
  FFlatLineColor := Value;
  Invalidate;
end;

procedure TAdvChartSpinEdit.CMShowingChanged(var Message: TMessage);
begin
  inherited;
...
procedure TAdvChartSpinEdit.CMTextChanged(var Message: TMessage);
begin
  if FBlockChange then
    Exit;
  inherited;
...
procedure TAdvChartSpinEdit.CMMouseEnter(var Msg: TMessage);
var
  pf: TCustomForm;
begin
  inherited;
...
procedure TAdvChartSpinEdit.CMMouseLeave(var Msg: TMessage);
begin
  inherited;
...
procedure TAdvChartSpinEdit.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
end;

procedure TAdvChartSpinEdit.WMKeyDown(var Msg:TWMKeydown);
begin
  inherited;
...
procedure TAdvChartSpinEdit.WMChar(var Msg: TWMKey);
var
  key: Char;
  s:string;
  OldSS,OldSL: integer;
...
procedure TAdvChartSpinEdit.SetShowSeconds(const Value: Boolean);
var
  TV: TDateTime;
begin
...
procedure TAdvChartSpinEdit.SetLabelAlwaysEnabled(const Value: Boolean);
begin
...
procedure TAdvChartSpinEdit.SetEnabled(Value: Boolean);
var
  OldValue: Boolean;
begin
  OldValue := Enabled;
...
procedure TAdvChartSpinEdit.SetEditAign(const Value: TEditAlign);
begin
  FEditAlign := Value;
  ReCreateWnd;
end;

function TAdvChartSpinEdit.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TAdvChartSpinEdit.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER,REL_VER),MakeWord(MIN_VER,MAJ_VER));
end;

procedure TAdvChartSpinEdit.SetVersion(const Value: string);
begin

end;

procedure TAdvChartSpinEdit.Init;
begin
  FNormalColor := Color;
  FFontColor := Font.Color;
...
destructor TAdvChartTimerSpeedButton.Destroy;
begin
...
procedure TAdvChartTimerSpeedButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TAdvChartTimerSpeedButton.MouseUp(Button: TMouseButton; Shift: TShiftState;
                                  X, Y: Integer);
begin
...
procedure TAdvChartTimerSpeedButton.TimerExpired(Sender: TObject);
begin
...
procedure TAdvChartTimerSpeedButton.Paint;
const
...
begin
  ...
  if (TAdvChartSpinButton(Owner).ButtonColor <> clNone) then
  begin
    if FState = bsDown then
      clr := TAdvChartSpinButton(Owner).ButtonColorDown
    else
      if FHasMouse then
        clr := TAdvChartSpinButton(Owner).ButtonColorHot
      else
        clr := TAdvChartSpinButton(Owner).ButtonColor;
...
constructor TAdvChartTimerSpeedButton.Create(AOwner: TComponent);
var
  dwVersion:Dword;
...
function TAdvChartTimerSpeedButton.DoVisualStyles: Boolean;
begin
  if FIsWinXP then
...
procedure TAdvChartTimerSpeedButton.CMMouseEnter(var Message: TMessage);
begin
  inherited;
  FHasMouse := True;
  Invalidate;
...
procedure TAdvChartTimerSpeedButton.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  FHasMouse := False;
  Invalidate;
