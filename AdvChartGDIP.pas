...
uses
  Windows, ActiveX, Math, Graphics, Classes, Controls , SysUtils
  {$IFDEF DELPHIXE3_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
\**************************************************************************)
  ...
  TChartGradientType = (gtSolid, gtRadial, gtVertical, gtHorizontal, gtForwardDiagonal, gtBackwardDiagonal, gtAngle, gtHatch, gtPath, gtTexture, gtNone);
...
resourcestring
  ADVCHARTGDIPERRMSG = 'Could not allocate memory for image';
...
begin
  ...
  begin
    ms.Free;
    raise Exception.Create(ADVCHARTGDIPERRMSG);
  end;
...
procedure TChartGDIPPicture.MakeBitmapTransparent(ABMP: IWICBitmap);
var
...
function TChartGDIPPicture.Create2DBitmap(ACanvas: TDirect2DCanvas): ID2D1Bitmap;
var
  hglobal: THandle;
  gStream: IStream;
  pcbWrite: Longint;
...
function TChartGDIPPicture.GetImageSizesFromFileName(FileName: String): boolean;
var
  sExt: String;
  w, h: Word;
begin
...
procedure TChartGDIPPicture.GDIPDraw(Graphics: TGPGraphics; const Rect: TGPRectF);
var
  multi: TGPImage;
  pstm: IStream;
  hGlobal: THandle;
